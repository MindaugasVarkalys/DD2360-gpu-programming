...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out) {
    ...
    const int idx_x = threadIdx.x + blockDim.x * blockIdx.x;
    const int idx_y = threadIdx.y + blockDim.y * blockIdx.y;

    // make sure that we don't step out of bounds
    if (idx_x >= width || idx_y >= height) return;

    int offset = width * idx_y + idx_x;
